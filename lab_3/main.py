...
class MainWindow(QWidget):
    ...
    def initUI(self):
        ...
        lbl1 = QLabel('Лабораторна робота №3 - \nМногочлен-Лагранжа - \n25 варіант')
        lbl2 = QLabel('Студент групи ІВ-82')
        lbl3 = QLabel('Троценко\nДанііл\nАнатолійович')

        btn1 = QPushButton('Показати графіки')
    ...
    def Lag(self):
        ...
        ax.plot(x,y, label='x^2*cos(x)')
        ax.plot(xnew,ynew,'o', label='Інтерполяція Многочленом Лагранжа')


        errors = [ynew[i] - y[i] for i in range(len(y))]
        ax.plot(x, errors, label='Похибка')
